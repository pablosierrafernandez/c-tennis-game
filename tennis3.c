...
/*     El segon parametre fa referencia al nombre de pilotes que es jugaran. */
/*									     */
/*  Compilar i executar:					  	     */
...
int cont=-1;
int numPaletas=0;
int numPelotas, numMaxPelotas;
int marc_u=0, marc_o=0;

int id_win, i, id_tecla, id_cont;
int *p_win, *p_n_moviments, *p_tecla, *p_cont;
*/
int retard, ipc_mem,  ipc_tec,  ipc_numPelotas,ipc_paletas,;			/* valor del retard de moviment, en mil.lisegons, ipc_paletas, p_paletas*/
int *p_mem, *p_tec, *p_numPelotas, *p_paletas;
 


pid_t tpid[MAX_PALETAS];
...
int inicialitza_joc(void)
{
  ...
  ipc_mem = ini_mem(retwin);  /* crear zona mem. compartida */
  p_mem = map_mem(ipc_mem);   //obtenir adres. de mem. compartida HACER ESTO PERO CON N_pelotas?
...
void * moure_pilota(void * cap){
  ...
  do{
   ...
   if (cont!=-1){
     if (cont==0){
	cont=-1;
      /* Pilota apareix a meitat paleta usuari, augmentem marcador ordinador i modifiquem numPelotas*/
	pthread_mutex_lock(&mutex_varg);
	numPelotas--;
	marc_o++;
	pil_pc=ipu_pc+1;
	pil_pf=(l_pal/2)+ipu_pf;
	pthread_mutex_unlock(&mutex_varg);
	ipil_pf = pil_pf; ipil_pc = pil_pc;				/* fixar valor real posicio pilota */
	pil_vf = -pil_vf; pil_vc = -pil_vc;
	pthread_mutex_lock(&mutex_pant);;
  	win_escricar(ipil_pf,ipil_pc,'.',INVERS); /* imprimeix pilota */
	pthread_mutex_unlock(&mutex_pant);
     }
     if (cont>0){
	cont=-1;
       /* Pilota apareix a meitat paleta ordinador, augmentem marcador usuari i modifiquem numPelotas*/
	pthread_mutex_lock(&mutex_varg);
	numPelotas--;
	marc_u++;
	pil_pc=(paletas_or[0].ipo_pc)-1;
	pil_pf=(l_pal/2)+(paletas_or[0].ipo_pf);
	pthread_mutex_unlock(&mutex_varg);
	ipil_pf = pil_pf; ipil_pc = pil_pc;				/* fixar valor real posicio pilota */
	pil_vf = -pil_vf; pil_vc = -pil_vc;
	pthread_mutex_lock(&mutex_pant);
...
int main(int n_args, const char *ll_args[]){
  ...
  time_t inicio, fin;
  char buffer[20], str_ipc_paletas[20], str_n_fil[3], str_n_col[3], str_l_pal[3], str_ipo_pf[4], str_ipo_pc[4], str_po_pf[4], str_v_pal[4],str_ipc_mem[20], str_ipc_tec[20], str_ipc_numPelotas[10],  str_index[1];


  
  if ((n_args != 2) && (n_args !=3) && (n_args !=4)){
    fprintf(stderr,"Comanda: tennis2 fit_param [numPelotas] [retard]\n");
  	exit(1);
  }
  ...
  if (n_args == 4){
   ...
   numMaxPelotas = atoi (ll_args[2]);
   numPelotas=numMaxPelotas;
  }
  else{
   retard = 100;
   numMaxPelotas = atoi (ll_args[2]);
   ...
   ipc_paletas=ini_mem(sizeof(int)); 				/*crear zona mem compartida */
   p_paletas=map_mem(ipc_paletas); 				/* obtener dir.de mem. compartida*/
   *p_paletas=numPaletas;					/*inicializa variable compartida*/
   
   ipc_tec=ini_mem(sizeof(int)); 				/*crear zona mem compartida */
   p_tec=map_mem(ipc_tec);
   *p_tec = 0;
   ipc_numPelotas=ini_mem(sizeof(int)); 				/*crear zona mem compartida */
   p_numPelotas=map_mem(ipc_numPelotas);
   *p_numPelotas= numPelotas;
//   win_set(p_win, n_fil, n_col);
  ...
  pthread_mutex_init(&mutex_pant, NULL);			/* creacio mutex per escriure a pantalla */
  pthread_mutex_init(&mutex_varg, NULL);			/* creacio mutex per consultar variables globals */
...
sprintf(str_ipc_mem, "%i", ipc_mem);
sprintf(str_ipc_paletas, "%i", ipc_paletas);
sprintf(str_ipc_tec, "%i", ipc_tec);
sprintf(str_ipc_numPelotas, "%i", ipc_numPelotas);
for(i=0; i<numPaletas; i++){					/* creacio threads paletes ordinador */
	tpid[n] = fork();					/*crea un nuevo proceso*/
	if(tpid[n] == (pid_t) 0){				/* si es hijo entra*/
		sprintf(str_ipo_pf, "%i", paletas_or[i].ipo_pf);
		sprintf(str_ipo_pc, "%i", paletas_or[i].ipo_pc);	/*transformacion a string de los datos de la paleta*/
      		sprintf(str_po_pf, "%f", paletas_or[i].po_pf);
      		sprintf(str_v_pal, "%f", paletas_or[i].v_pal);
     		sprintf(str_index, "%i", i+1);
		execlp("./pal_ord3", "pal_ord3",str_ipo_pf, str_ipo_pc, str_po_pf, str_v_pal, str_n_fil,  str_n_col, str_l_pal, str_ipc_mem, str_ipc_paletas, str_ipc_tec, str_ipc_numPelotas, str_index, (char *)0);
		fprintf(stderr,"error: no puedes executar el proceso hijo \'pal_ord3\'\n");
     		exit(0);
        }
	else if (tpid[n] > 0) n++;				/*pertenece al proceso padre*/
}
  ...
  while(tec!=TEC_RETURN  && numPelotas>0){
    ...
    min=seg/60;
    seg=seg%60;
    /* Actualitzacio del marcador, numPelotas i temps */
    pthread_mutex_lock(&mutex_varg);
    sprintf(buffer,"U %d - %d O P:%d %2d:%2d",marc_u, marc_o, numPelotas, min, seg);
    pthread_mutex_unlock(&mutex_varg);

    pthread_mutex_lock (&mutex_pant);
    win_escristr(buffer);
    pthread_mutex_unlock (&mutex_pant);
    win_retard(retard);			/*ejecución con tiempo de retardo*/
    win_update();			/*refresca contenido ventana*/
  }
  ...
  pthread_mutex_destroy(&mutex_pant);				/* eliminacio mutex per escriure a pantalla */
  pthread_mutex_destroy(&mutex_varg);				/* eliminacio mutex per consultar variables globals */
  ...
  if (tec == TEC_RETURN) printf("S'ha aturat el joc amb la tecla RETURN! S'ha jugat durant: %2d:%2d \n", min, seg);
	...
	if (marc_o>marc_u) printf("Ha guanyat l'ordinador! Temps de joc %2d:%2d \n", min, seg);
        else if (marc_o==marc_u) printf("Ha estat empat s'han acabat les pilotes! Temps de joc %2d:%2d \n", min, seg);
	else printf("Ha guanyat l'usuari! Temps de joc: %2d:%2d \n", min, seg);
  }
elim_mem(ipc_paletas);			/*elimina zona memoria compartida*/
win_fi();
return(0);
}
